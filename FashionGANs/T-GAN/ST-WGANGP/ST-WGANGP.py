...
ITERS = 0
FID_MODEL = load_FID()
# IXED_IMAGES, TARGETS = projector_img_labs(dataset=dataset, b_size=100, n_workers=2)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_dataloader, 0):
        ...
        for _ in range(critic_iter):
            ...
            real_critic = netD(real_cpu, sequence_embeddings).view(-1)
            ...
            fake_critic = netD(fake.detach(), sequence_embeddings).view(-1)
            D_x = real_critic.mean().item()
            D_G_z1 = fake_critic.mean().item()

            # WLoss function + WGP
            gp = gradient_penalty(discriminator=netD, l=sequence_embeddings, labels=True, real_batch=real_cpu, fake_batch=fake,
                                  device=device)
