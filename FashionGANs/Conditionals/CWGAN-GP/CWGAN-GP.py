...
netG = Generator(ngpu, nc, nz, ngf, number_classes, embedding_size).to(device)
...
for bs in batch_size:
    ...
    dataset, dataloader = load_dataset(net=network, dataset_name=dataset_name, data_root=DATA_ROOT, transform=TRANSFORM,
                                       dictionary=dictionary, workers=n_workers, batch_size=bs)
    ...
    for lr in learning_rates:
        ...
        directory = "/app/gan_results/previous results/"
        OUT_PATH = directory + network + "/" + dataset_name + "/" + f"{num_epochs}" + "/" + opt + "/" + f"{bs}" + "/" + f"{lr}"
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                for _ in range(critic_iter):
                    ...
                    # WLoss function + WGP
                    gp = gradient_penalty(discriminator=netD, l=y, labels=True, real_batch=real_cpu, fake_batch=fake, device=device)
